...
def run_bot_task(spreadsheet_id, worksheet_name=None, selected_lotteries=None, max_consecutive_failures=3, restart_mode='minutes', restart_minutes=30, restart_datetime=None, start_row=None, end_row=None):
    ...
    try:
        ...
        if start_row and end_row:
            log_message(f"📊 Found {total_email_count} email(s) in Google Spreadsheet (rows {start_row}-{end_row})", 'info')
        else:
                            ...
                            if restart_mode == 'minutes':
                                ...
                                if restart_datetime:
                                    try:
                                        ...
                                        log_message(f"❌ Error parsing restart datetime: {e}. Using default 30 minutes...", 'error')
                                    ...
                                    restart_seconds = 1800
                                    restart_time = datetime.now() + timedelta(minutes=30)
                                    log_message(f"⏰ Bot will automatically restart after 30 minutes (default)...", 'info')
        ...
        if bot_status['running']:
            ...
            bot_status['current_step'] = '完了'
        else:
            log_message("⏹️ ユーザーによってボットが停止されました", 'warning')
            bot_status['current_step'] = '停止'
...
def start_bot():
    ...
    if bot_status['running']:
        return jsonify({'success': False, 'message': 'ボットは既に実行中です'}), 400
    
    # Get Google Spreadsheet ID
    spreadsheet_id = request.form.get('spreadsheet_id', '').strip()
    if not spreadsheet_id:
        return jsonify({'success': False, 'message': 'GoogleスプレッドシートIDまたはURLを入力してください'}), 400
    ...
    if start_row_str or end_row_str:
        ...
        if not start_row_str or not end_row_str:
            return jsonify({'success': False, 'message': '範囲を指定する場合は、開始行と終了行の両方を入力してください'}), 400
        ...
        try:
            ...
            if start_row < 1:
                return jsonify({'success': False, 'message': '開始行は1以上である必要があります'}), 400
            if end_row < 1:
                return jsonify({'success': False, 'message': '終了行は1以上である必要があります'}), 400
            if start_row > end_row:
                return jsonify({'success': False, 'message': '開始行は終了行以下である必要があります'}), 400
        except (ValueError, TypeError):
            return jsonify({'success': False, 'message': '無効な行範囲です。有効な数値を入力してください'}), 400
    ...
    try:
        ...
        if max_consecutive_failures < 1 or max_consecutive_failures > 20:
            return jsonify({'success': False, 'message': '最大連続失敗回数は1から20の間である必要があります'}), 400
    except (ValueError, TypeError):
        return jsonify({'success': False, 'message': '無効な最大連続失敗回数です。1から20の間の数値を入力してください'}), 400
    ...
    if restart_mode == 'minutes':
        try:
            ...
            if restart_minutes < 1:
                return jsonify({'success': False, 'message': '再起動分数は1以上である必要があります'}), 400
        except (ValueError, TypeError):
            return jsonify({'success': False, 'message': '無効な再起動分数です。有効な数値を入力してください'}), 400
    elif restart_mode == 'datetime':
        restart_datetime = request.form.get('restart_datetime')
        if not restart_datetime:
            return jsonify({'success': False, 'message': '再起動の日時を選択してください'}), 400
        try:
            # Validate datetime format and check if it's in the future
            restart_dt = datetime.strptime(restart_datetime, '%Y-%m-%dT%H:%M')
            if restart_dt <= datetime.now():
                return jsonify({'success': False, 'message': '再起動日時は未来の日時である必要があります'}), 400
        except ValueError:
            return jsonify({'success': False, 'message': '無効な日時形式です。YYYY-MM-DDTHH:MM形式を使用してください'}), 400
    ...
    return jsonify({'success': True, 'message': 'ボットが正常に開始されました'})
...
def stop_bot():
    ...
    if not bot_status['running']:
        return jsonify({'success': False, 'message': 'ボットは実行されていません'}), 400
    
    bot_status['running'] = False
    log_message("⏹️ 停止リクエストを受信しました", 'warning')
    
    return jsonify({'success': True, 'message': '停止リクエストを送信しました'})
...
def download_logs():
    ...
    try:
        ...
        if os.path.exists(log_filename):
            ...
            return jsonify({'success': False, 'message': 'ログファイルが見つかりません'}), 404
    except Exception as e:
...
def check_spreadsheet():
    ...
    try:
        ...
        if not spreadsheet_id:
            return jsonify({'success': False, 'message': 'スプレッドシートIDが必要です'}), 400
